...
class Campaigns(Base):
    ...
    campaign_name = Column(
        String(255), nullable=False, index=True, doc="Name of the campaign"
    )
    description = Column(Text, doc="Detailed description of campaign goals")
    start_date = Column(Date, nullable=False, index=True, doc="Campaign start date")
    end_date = Column(Date, index=True, doc="Campaign end date (nullable for ongoing)")
    ...
    budget = Column(Numeric(10, 2), doc="Campaign budget allocation")
    target_metrics = Column(JSONB, doc="JSON object containing target KPIs and goals")
    actual_metrics = Column(
        JSONB, doc="JSON object containing actual performance metrics"
    )
    created_at = Column(
    ...
    target_specialties = relationship(
        "CampaignTargetSpecialties",
        back_populates="campaign",
        cascade="all, delete-orphan",
    )
    campaign_relationships = relationship(
        "CampaignRelationships", back_populates="campaign"
    )
...
class CampaignTargetSpecialties(Base):
    ...
    # Indexes
    __table_args__ = (Index("idx_campaign_specialties_specialty", "specialty_name"),)
...
class Relationships(Base):
    ...
    lead_score = Column(Integer, doc="Numerical value (1-5) indicating lead potential")
    last_activity_date = Column(
        DateTime(timezone=True),
        index=True,
        doc="Date of the most recent associated activity",
    )
    next_steps = Column(Text, doc="Brief description of the immediate next actions")
    ...
    relationship_status_type = relationship(
        "RelationshipStatusTypes", back_populates="relationships"
    )
    loyalty_status_type = relationship(
        "LoyaltyStatusTypes", back_populates="relationships"
    )
    campaign_relationships = relationship(
        "CampaignRelationships", back_populates="relationship"
    )
    history = relationship(
        "RelationshipHistory",
        back_populates="related_relationship",
        order_by="RelationshipHistory.changed_at.desc()",
    )
...
class CampaignRelationships(Base):
    ...
    relationship = relationship(
        "Relationships", back_populates="campaign_relationships"
    )

    # Indexes
    __table_args__ = (Index("idx_campaign_relationships_assigned", "assigned_at"),)
...
class ContentLibrary(Base):
    ...
    title = Column(
        String(255), nullable=False, index=True, doc="Title of the content asset"
    )
    description = Column(Text, doc="Brief description of the content")
    file_path = Column(
        Text, nullable=False, doc="URL or path to the stored content file"
    )
    content_type = Column(
        String(50),
        nullable=False,
    ...
    tags = Column(ARRAY(String), index=True, doc="Array of keywords for searching")
    uploaded_by_user_id = Column(
        Integer, ForeignKey("sf_users.id"), doc="User who uploaded the content"
    )
    created_at = Column(
        DateTime(timezone=True),
        default=datetime.now,
...
class RelationshipHistory(Base):
    ...
    previous_relationship_status = Column(
        String(50), doc="Previous relationship status"
    )
    new_relationship_status = Column(String(50), doc="New relationship status")
    previous_loyalty = Column(String(50), doc="Previous loyalty status")
    new_loyalty = Column(String(50), doc="New loyalty status")
    change_reason = Column(Text, doc="Reason for the status change")
    changed_at = Column(
        DateTime(timezone=True),
        default=datetime.now,
        nullable=False,
    ...
    # Indexes
    __table_args__ = (Index("idx_relationship_history_changed", "changed_at"),)
...
class RelationshipMetrics(Base):
    ...
    metric_date = Column(
        Date, nullable=False, index=True, doc="Date for which metrics are calculated"
    )
    referrals_count = Column(
        Integer, default=0, doc="Number of referrals from this relationship"
    )
    meetings_count = Column(Integer, default=0, doc="Number of meetings held")
    calls_count = Column(Integer, default=0, doc="Number of phone calls made")
    emails_count = Column(Integer, default=0, doc="Number of emails sent")
    engagement_score = Column(Numeric(5, 2), doc="Calculated engagement score")
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "relationship_id", "metric_date", name="uq_relationship_metric_date"
        ),
    )
...
class Reminders(Base):
    ...
    linked_entity_type = Column(
        ...
        nullable=False,
        index=True,
        doc="Type of entity the reminder is linked to",
    )
    linked_entity_id = Column(
        UUID(as_uuid=True), nullable=False, index=True, doc="ID of the linked entity"
    )
    ...
    description = Column(
        Text, nullable=False, doc="Description of what needs to be done"
    )
    due_date = Column(
        DateTime(timezone=True),
        nullable=False,
    ...
    completed_at = Column(
        DateTime(timezone=True), doc="When the reminder was completed"
    )
    created_at = Column(
        DateTime(timezone=True),
        default=datetime.now,
    ...
    __table_args__ = (
        ...
        Index("idx_reminders_entity", "linked_entity_type", "linked_entity_id"),
...
class SalesforceSync(Base):
    ...
    entity_type = Column(
        String(50), nullable=False, index=True, doc="Type of entity being synced"
    )
    local_id = Column(UUID(as_uuid=True), nullable=False, doc="Local database ID")
    sf_id = Column(String(18), nullable=False, index=True, doc="Salesforce ID")
    last_synced_at = Column(
        DateTime(timezone=True), doc="Last successful sync timestamp"
    )
    sync_status = Column(
        String(50), doc="Status of sync ('success', 'failed', 'pending')"
    )
    error_message = Column(Text, doc="Error details if sync failed")
...
class LLMProcessingQueue(Base):
    ...
    entity_type = Column(
        String(50), nullable=False, index=True, doc="Type of entity to process"
    )
    entity_id = Column(
        UUID(as_uuid=True), nullable=False, doc="ID of entity to process"
    )
    ...
    status = Column(String(50), default="pending", index=True, doc="Processing status")
    queued_at = Column(
        DateTime(timezone=True),
        default=datetime.now,
        nullable=False,
    ...
    processed_at = Column(DateTime(timezone=True), doc="When item was processed")
    error_message = Column(Text, doc="Error details if processing failed")

    # Indexes
    __table_args__ = (
        Index("idx_llm_queue_status_priority", "status", "priority"),
        Index("idx_llm_queue_entity", "entity_type", "entity_id"),
    )
...
class NextBestActions(Base):
    ...
    action_type = Column(
        String(50), nullable=False, index=True, doc="Type of recommended action"
    )
    action_description = Column(
        Text, nullable=False, doc="Detailed description of the recommended action"
    )
    confidence_score = Column(Numeric(3, 2), doc="AI confidence score (0.00-1.00)")
    suggested_date = Column(Date, doc="Suggested date to take the action")
    is_completed = Column(
        Boolean, default=False, index=True, doc="Whether the action has been completed"
    )
    generated_at = Column(
        DateTime(timezone=True),
        default=datetime.now,
    ...
    expires_at = Column(DateTime(timezone=True), doc="When the recommendation expires")

    # Relationships
    related_relationship = relationship(
        "Relationships", back_populates="next_best_actions"
    )

    # Indexes
    __table_args__ = (
        Index(
            "idx_next_best_actions_active",
            "relationship_id",
            "is_completed",
            "expires_at",
        ),
