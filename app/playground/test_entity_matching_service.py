...
def test_multiple_entity_types():
    ...
    results = service.match_multiple(
        query,
        entity_types,
        with_scoring=True
    )
    ...
    for entity_type, matches in results.items():
        ...
        for match in matches[:2]:
            print(f"  - {match.display_name} (confidence: {match.confidence_score:.2f})")
...
def test_best_match():
    ...
    best_match = service.get_best_match(
        query,
        EntityType.CONTACT,
        min_confidence=0.8
    )
...
def test_context_boosting():
    ...
    for context in contexts:
        print(f"\nQuery: '{query}'")
        if context:
            print(f"Context: organization={context.organization}, specialty={context.specialty}")
        else:
            print("Context: None")
