...
from workflows.deprecated_workflows.forward_email_parsing_workflow import (
    ForwardEmailParsingWorkflow,
)
...
def list_emails():
    ...
    for session in db_session():
        ...
        for email in emails:
            subject = (
                email.subject[:37] + "..." if len(email.subject) > 40 else email.subject
            )
            from_email = (
                email.from_email[:17] + "..."
                if len(email.from_email) > 20
...
def test_parse_forwarded_email():
    ...
    for session in db_session():
        ...
        email = (
            session.query(Email)
            .filter(Email.subject.ilike("%Fwd:%"))
...
def test_parse_direct_email():
    ...
    for session in db_session():
        ...
        try:
            ...
            email_type = result.nodes.get("EmailTypeDetectionNode", {}).get(
                "email_type"
            )
